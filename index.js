...
var argv = minimist(process.argv.slice(2), { string: ["channel", "port"], boolean: ["test"] });
...
if (Object.keys(argv).length > 2) {
  ...
  try {
    if (!fs.existsSync(configFilePath)) {
      console.error("ERROR: config.json does not exist. Please create a config.json file and try again.");
      process.exit(1);
    }
  } catch (err) {
    console.error(err);
  ...
  // check if they begin with DISHORDE_,
  // then create an object from matching key-value pairs
  const envVars = Object.entries(process.env).reduce((acc, [key, value]) => {
    if (key.indexOf("DISHORDE_") !== -1) {
      let configKey = key.replace("DISHORDE_", "");
  ...
  // Create a list of sensitive options,
  // so we can avoid printing these to the console
  ...
  for (const envVarKey in envVars) {
    ...
    console.warn(`Overriding config option '${envVarKey}' with value '${sensitiveOptions.includes(envVarKey) ? '<REDACTED>' : envVarValue}'`);
    config[envVarKey] = envVarValue;
  }
}

// TODO: Actually do something in the test mode,
//       to confirm the application is working etc.
var testMode = argv["test"];
if (testMode) {
  console.warn("Running in test mode!");
  console.warn("Test mode finished, exiting ...");
  process.exit(0);
}
